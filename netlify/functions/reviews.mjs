...
export default async (req) => {
  ...
  if (req.method === "GET") {
    ...
    for (const blob of listed.blobs || []) {
      if (!blob.key || blob.key.startsWith("rl:")) continue;
      const item = await store.get(blob.key, { type: "json" });
      if (item && item.rating && item.product) reviews.push(item);
    }
  ...
  if (req.method === "POST") {
    ...
    if (comment.length < 8) return json({ error: "Please write a short review (at least 8 characters)." }, 400);
